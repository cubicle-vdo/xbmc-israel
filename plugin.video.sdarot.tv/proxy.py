    ...
    def _process_request(self, data):
        ...
        if self.request.state == HTTP_PARSER_STATE_COMPLETE:
            ...
            # and queue for the server with appropriate headers
                ...
                # Wait for previous session to close
                if self.request.method == 'GET' and 'range' in self.request.headers:
                    time.sleep(1)
...
class TCP(object):
    ...
    def __init__(self, hostname='127.0.0.1', port=PROXY_PORT, backlog=100):
    ...
    def stop(self):
        ...
        closer = Server(self.hostname, self.port)
        try:
            closer.connect()
        finally:
            closer.close()
