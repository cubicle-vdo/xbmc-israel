...
__settings__ = xbmcaddon.Addon(id='plugin.video.makoVOD.video')
...
__properties = {'pKey':'81d42db7c211bf9615a895c504', 'accountId': '39', 'broadcasterId':'24', 'bundle':'com.keshet.mako.VODandroid'}
import APCategoryLoader, APAccountLoader, APBroadcaster, APCategoryList, APItemLoader
import APEpgLoader, APVodItem, APCategory
...
def addCategoryView(category):
    ...
    if __DEBUG__:
        xbmc.log('Thumbnail Url--> %s' % (thumbnail), xbmc.LOGERROR)
        xbmc.log('Fanart Url--> %s' % (fanart), xbmc.LOGERROR)
...
def playMovie(item):
    ...
    listItem = xbmcgui.ListItem(title, iconImage = thumbnail, thumbnailImage = thumbnail, path=_url) # + '|' + 'Mozilla/5.0 (Windows; U; Windows NT 5.1; en-GB; rv:1.9.0.3) Gecko/2008092417 Firefox/3.0.3')
    listItem.setInfo(type='Video', infoLabels={ "Title": urllib.unquote(title), "Plot": urllib.unquote(summary)})
    listItem.setProperty('IsPlayable', 'true')
