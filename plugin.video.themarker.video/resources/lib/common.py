...
def getData(url, timeout=__cachePeriod__, name=''):
        ...
        if name == '':
            ...
            cachePath = xbmc.translatePath(os.path.join(__PLUGIN_PATH__, 'cache', 'pages', "name_"+name))
        if timeout > 0:
        ...
        try:     
            ...
            if (timeout > 0) or ( 1 ):
                if __DEBUG__:
                    print 'Writing cache file ' + cachePath
                f = open(cachePath, 'wb')
                f.write(data)
                f.close()
            if __DEBUG__:
            ...
            xbmc.log('Error in getData: Unable to save cache to ' + cachePath, xbmc.LOGERROR)
            return 'unavailable'
