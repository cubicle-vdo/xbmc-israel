...
'''
This module provides the main API for accessing the gozlanurlresolver features.

For most cases you probably want to use :func:`gozlanurlresolver.resolve` or 
:func:`gozlanurlresolver.choose_source`.
...
from plugnplay.interfaces import gozlanurlresolver
...
def resolve(web_url):
    ...
    It is usually as simple as::
        ...
        media_url = gozlanurlresolver.resolve(web_url) 
    ...
    Behind the scenes, :mod:`gozlanurlresolver` will check each of the available 
...
def choose_source(sources):
    ...
    Example::
		...
		source = gozlanurlresolver.choose_source(sources)
		if source:
...
def display_settings():
    '''
    Opens the settings dialog for :mod:`gozlanurlresolver` and its plugins.
    
    This can be called from your addon to provide access to global 
    :mod:`gozlanurlresolver` settings. Each resolver plugin is also capable of 
    exposing settings.
    
    .. note::
    
        All changes made to these setting by the user are global and will 
        affect any addon that uses :mod:`gozlanurlresolver` and its plugins.
    '''
